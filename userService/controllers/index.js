...
exports.createUser = async (req, res, next) => {
  try {
    ...
    const data = await axios.put("http://localhost:4000/dailyPass/user", {
...
exports.getSingleUserDetail = async (req, res, next) => {
  try {
    ...
    let promisesArray = [];
    promisesArray.push(
      axios.get(`http://localhost:4000/dailypass/details?userId=${userID}`)
    );

    promisesArray.push(axios.get(`http://localhost:3000/series/all`));
    promisesArray.push(
      userCrud.getOne({
        findBy: { _id: userID },
